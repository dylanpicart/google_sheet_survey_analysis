...
def test_map_overarching_workflow(tmp_path):
    ...
    pd.DataFrame(
        {"Raw Question": ["How safe do you feel? [Safety]", "Is there an adult you trust? [Trusted Adult]"]}
    ).to_csv(audit_younger, index=False)
    pd.DataFrame({"Raw Question": ["I feel connected [Connection]", "Adults talk about race [Race]"]}).to_csv(
        audit_older, index=False
    )
    ...
    pd.DataFrame({"Canonical Question": ["safety", "trusted adult"], "SomeCol": [1, 2]}).to_csv(
        summary_younger, index=False
    )
    pd.DataFrame({"Canonical Question": ["connection", "race"], "SomeCol": [3, 4]}).to_csv(summary_older, index=False)
