...
def test_consolidate_summary(tmp_path):
    ...
    df = pd.DataFrame(
        {"Column": ["Q1", "Q2"], "Agree": [3, 1], "Disagree": [0, 2], "Neither Agree nor Disagree": [1, 1]}
    )
    df.to_csv(input_csv, index=False)

    # Minimal dummy summary_map (bucket mapping) and overarching mapping
    summary_map = {"agree": "Agree", "disagree": "Disagree", "neither agree nor disagree": "Neither Agree Nor Disagree"}
    canon_to_over_map = {"q1": "Safety", "q2": "Trust"}
