...
def test_summarize_totals(tmp_path):
    ...
    pd.DataFrame({
        "Canonical Question": ["A", "B"], "Agree": [1, 2], "Disagree": [3, 4], "Overarching": ["X", "Y"], "School Year": ["21-22", "21-22"]
    }).to_csv(path_younger, index=False)
    pd.DataFrame({
        "Canonical Question": ["A", "B"], "Agree": [4, 1], "Disagree": [1, 1], "Overarching": ["X", "Y"], "School Year": ["22-23", "22-23"]
    }).to_csv(path_older, index=False)

    # Run function
    df_out = summarize_totals(
        consolidated_younger_fp=str(path_younger),
        consolidated_older_fp=str(path_older),
