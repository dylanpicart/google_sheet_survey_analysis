...
def dummy_en_sp_mapping():
    ...
    return {"¿Está satisfecho?": "Satisfied?", "¿Confía en adultos?": "Trust?", "¿Cómo se siente?": "How do you feel?"}


@pytest.fixture
def dummy_answer_mapping():
    """Spanish cell value mapping."""
    return {"Sí": "Yes", "No": "No", "Tal vez": "Maybe", "Satisfecho": "Satisfied", "Insatisfecho": "Unsatisfied"}


@pytest.fixture
def dummy_links_yaml(tmp_path):
    """Dummy links.yaml structure for extract tests."""
    links = {"2023": {"younger": {"Test School": {"Form Responses": {"sheet_id": "sid", "gid": "gid"}}}}}
...
def dummy_dataframe():
    ...
    return pd.DataFrame({"Satisfied?": ["Yes", "No", "Yes"], "Trust": ["Yes", "No", "Maybe"]})
...
def dummy_scale_orders():
    ...
    return {
        "likert_1": ["Strongly Disagree", "Disagree", "Neither Agree nor Disagree", "Agree", "Strongly Agree"],
