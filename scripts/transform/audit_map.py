...
def build_accepted_map(question_df, RAW_QUESTION_COL):
    accepted_map = {
        row[RAW_QUESTION_COL]: (
            set(str(row["Sample Responses"]).split("|")) if pd.notna(row["Sample Responses"]) else set()
        )
...
def audit_map_for_group(group, params, school_years=None, meta_variants=None, junk_cols=None):
    ...
    for year in school_years:
        ...
        if dropped_questions:
            logger.warning(
                f"  ⚠️ Dropped {len(dropped_questions)} canonical questions due to all-NaN: \
                {dropped_questions[:3]}{'...' if len(dropped_questions) > 3 else ''}"
            )
...
def main():
    for group, params in GROUP_PARAMS.items():
        audit_map_for_group(
            group=group, params=params, school_years=SCHOOL_YEARS, meta_variants=META_VARIANTS, junk_cols=junk_cols
        )
